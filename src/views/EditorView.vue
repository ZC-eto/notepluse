...
async function onToolbarFromMore(action: string) {
  closeToolbarMore()
  await onToolbar(action)
}

function onDocPointerDown(ev: MouseEvent) {
  if (!toolbarMoreOpen.value) return
  const t = ev.target as Node | null
  if (moreFlyoutBtn.value?.contains(t)) return
  const fly = document.getElementById('editor-more-flyout')
  if (fly?.contains(t as Node)) return
  closeToolbarMore()
}

function onDocKeyEscape(ev: KeyboardEvent) {
  if (ev.key === 'Escape' && toolbarMoreOpen.value) {
    closeToolbarMore()
  }
}
...
onMounted(() => {
  ...
  document.addEventListener('keydown', onDocKeyEscape, true)
})
...
onBeforeUnmount(() => {
  ...
  document.removeEventListener('keydown', onDocKeyEscape, true)
})
</script>
    ...
    <template v-else>
        ...
        <div class="editor-more-wrap">
          <button
            ref="moreFlyoutBtn"
            type="button"
            class="editor-tool-btn is-icon editor-more-trigger"
            :aria-expanded="toolbarMoreOpen"
            aria-haspopup="true"
            aria-controls="editor-more-flyout"
            title="更多格式"
            aria-label="更多格式"
            @mousedown.prevent
            @click="toggleToolbarMore"
          >
            <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="6" cy="12" r="1.6" fill="currentColor" /><circle cx="12" cy="12" r="1.6" fill="currentColor" /><circle cx="18" cy="12" r="1.6" fill="currentColor" /></svg>
          </button>
          <div
            v-if="toolbarMoreOpen"
            id="editor-more-flyout"
            class="editor-more-flyout"
            role="toolbar"
            aria-label="更多编辑格式"
          >
            <button
