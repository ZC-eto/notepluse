# 墨线工作台（Ink Ledger）UI 设计系统

> **状态**：设计规格 v1.0（仅设计，不改变现有源码）  
> **产品**：本地 Markdown 工作区；Markdown 是唯一真源，任务仅来自显式 Task Block。  
...
**墨线工作台不是把 Markdown 套进项目管理壳子；它是一张可书写的纸面，在同一条“工作线”上把笔记、任务、日期和进度投影出来。**
...
- Gantt 与 Calendar 直接承担“快速新建任务”，没有明确 Task Block 目标，可能诱导不安全回写；
...
```text
┌─────────────────────────────────────────────────────────────────────────────────────────────┐
│ Product bar: Ink Ledger · [⌘K 搜索/命令]             同步/保存状态 · Theme · Settings         │ 48
├───────────┬───────────────────────────────────────────┬─────────────────────────────────────┤
...
| `--color-warning` | `#9A651B` | 日期风险、未闭合 Task Block |
